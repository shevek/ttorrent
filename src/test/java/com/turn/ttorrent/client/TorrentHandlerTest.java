...
public class TorrentHandlerTest {
	...
	private TorrentHandler test(Torrent torrent, File parent) throws Exception {
		...
		TorrentHandler torrentHandler = new TorrentHandler(client, torrent,
				parent);
	...
	public void testMultiFileSeed() throws Exception {
		...
		TorrentCreator creator = TorrentTestUtils.newTorrentCreator(d_seed,
				12345678);
	...
	public void testMultiFileLeech() throws Exception {
		...
		assertEquals("We have no pieces.", 0,
				torrentHandler.getCompletedPieceCount());
		assertFalse("We are not complete, i.e. a seed.",
				torrentHandler.isComplete());
	}
